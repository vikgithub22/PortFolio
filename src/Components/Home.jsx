...
const Home = () => {
  return (
    <div name="home" className='h-screen w-full bg-gradient-to-b from-black to-gray-700 overflow-hidden relative'>
      ...
      >
        <motion.div
          className="absolute top-10 left-1/2 transform -translate-x-1/2 w-96 h-96 bg-gradient-to-b from-cyan-500 to-blue-500 rounded-full"
          animate={{ y: [0, 200, 0], x: [0, 100, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute top-1/4 left-1/4 w-56 h-56 bg-gradient-to-r from-yellow-700 to-orange-600 rounded-full"
          animate={{ y: [0, -150, 0], opacity: [0.5, 0.8, 0.5] }}
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-10 right-1/4 w-72 h-72 bg-gradient-to-l from-pink-700 to-purple-500 rounded-full"
      ...
      <div className='max-w-screen-lg mx-auto flex flex-col items-center justify-center h-full px-4 md:flex-row relative z-10'>
        <div className='flex flex-col justify-center h-full'>
          ...
          <h4 className='sm:text-2xl font-semibold text-cyan-500'>
            A Software Developer
          </h4>
          <p className='text-white py-1 max-w-md'>
        ...
        <div className='mt-10'>
          <img src={logo} alt="logo" className='z-10 w-60 h-60 ml-6 rounded-2xl  transform transition-all hover:scale-110 duration-500'/>
        </div>
      </div>
    </div>
  );
